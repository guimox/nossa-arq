...
export default function HomePage({ initialSlides }: HomePageProps) {
  ...
  return (
    <div className="mx-auto h-screen w-screen overflow-hidden bg-black">
      <div className="relative h-full w-full">
        ...
        <AnimatePresence initial={true} custom={direction} mode="wait">
          <motion.div
            ...
            className="absolute inset-0"
          >
            <div className="absolute inset-0 mx-auto flex max-w-[1700px] flex-col items-start justify-center gap-4 px-[1rem] py-2 md:gap-4 lg:px-0">
              <div className="flex flex-col overflow-visible py-4">
      ...
      <div className="absolute right-0 bottom-0 left-0 z-20 mx-auto flex w-full max-w-[1700px] flex-col-reverse items-start justify-between gap-10 px-[1rem] py-8 font-medium text-white md:flex-row md:items-center lg:px-0">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.6, ease: 'easeOut' }}
          className="border-b border-l p-4 text-base sm:p-6"
        >
          Autoral, brasileira e contemporânea — projetos que traduzem identidade,
          <br />
          pertencimento e beleza em cada detalhe.
        </motion.div>
        <div className="z-40 flex gap-2 sm:gap-3 md:gap-4">
          {slides.map((slide, index) => (
            <button
              key={index}
              onClick={() => handleColorBoxClick(index)}
              className={`z-40 h-4 h-6 w-4 w-6 cursor-pointer hover:opacity-80 ${
                slide.colors[index]
